...
function Invetationreceive(props) {
    ...
    <>
      {
        props.data.invitations.length > 0 ? <div className={styles.searchBox}>
          <input
            ...
            placeholder="Search By From User Name"
                ...
                <li className={`${styles.listingCard} ${styles.columnflex}`}>
                  ...
                  {item.toUsersList.map((frmlist) => (
                    ...
                    <>
                      {props.data.userid.indexOf(frmlist.userId) !==
                        -1 && frmlist.status === "null" ?
                        null : <Link href={"/meetingdetails/[meetingid]"} as={"/meetingdetails/" + item.meetingId}>
                        <a className={styles.Cardlink} onClick={() => props.meetingId(item.meetingId)} >link</a>
                      </Link>
                      }
                    </>
...
export default Invetationreceive;